...
class CitizenConfigurationTest {
	...
	@Mock
	private CitizenIntegrationProperties propertiesMock;

	@Test
	void testFeignBuilderCustomizer() {
		final var configuration = new CitizenIntegrationConfiguration();
		...
		try (final MockedStatic<FeignMultiCustomizer> feignMultiCustomizerMock = Mockito.mockStatic(FeignMultiCustomizer.class)) {
			...
			verify(clientRegistrationRepositoryMock).findByRegistrationId(CitizenIntegrationConfiguration.CLIENT_ID);
			...
			assertThat(errorDecoderCaptor.getValue()).hasFieldOrPropertyWithValue("integrationName", CitizenIntegrationConfiguration.CLIENT_ID);
